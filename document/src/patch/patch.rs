...
pub enum Patch {
	...
	ApplyStencilIXYZ(ApplyStencilIXYZPatch),
	...
	RestoreLayerCanvasIXYZ(RestoreLayerCanvasIXYZPatch),
...
impl Patch {
	pub fn target(&self) -> Uuid {
		match self {
			...
			Patch::ApplyStencilIXYZ(patch) => patch.target,
			...
			Patch::RestoreLayerCanvasIXYZ(patch) => patch.target,
...
impl IPatch for Patch {
	fn mode(&self) -> PatchMode {
		match self {
			...
			Patch::ApplyStencilI(patch) => patch.mode(),
			Patch::ApplyStencilIXYZ(patch) => patch.mode(),
			Patch::ApplyStencilUV(patch) => patch.mode(),
			Patch::ApplyStencilRGB(patch) => patch.mode(),
			Patch::ApplyStencilRGBA(patch) => patch.mode(),
			Patch::ApplyStencilRGBAXYZ(patch) => patch.mode(),
			...
			Patch::RestoreLayerCanvasI(patch) => patch.mode(),
			Patch::RestoreLayerCanvasIXYZ(patch) => patch.mode(),
			...
			Patch::RestoreLayerCanvasRGBAXYZ(patch) => patch.mode(),
