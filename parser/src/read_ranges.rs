	...
	async fn read_ranges(
		...
		let mut seek_sizes: Vec<(io::SeekFrom, u64)> = ranges
			...
			.map(|r| match (r.start_bound(), r.end_bound()) {
				// ..E => first E-1 bytes
				(Bound::Unbounded, Bound::Excluded(e)) => (io::SeekFrom::Start(0), *e as u64),
				// ..=E => first E bytes
				(Bound::Unbounded, Bound::Included(e)) => (io::SeekFrom::Start(0), (*e + 1) as u64),
				// -S.. => last S bytes
				(Bound::Included(s), Bound::Unbounded) if s < &0 => {
					(io::SeekFrom::End(*s), -*s as u64)
				}
				// S..E => S to E-1 bytes
				(Bound::Included(s), Bound::Excluded(e)) if s >= &0 && e >= &0 => {
					(io::SeekFrom::Start(*s as u64), (*e - *s) as u64)
				}
				// S..E => S to E-1 bytes
				(Bound::Included(s), Bound::Excluded(e)) if s < &0 && e < &0 => {
					(io::SeekFrom::End(*s as i64), (*e - *s) as u64)
				}
				// S..=E => S to E bytes
