...
fn build_macos() {
	...
	fs_extra::dir::copy(
		freeimage_native_dir,
		&out_dir,
		&fs_extra::dir::CopyOptions::new(),
	)
	.unwrap();
	let xcode_select_out: OsString = OsString::from_vec(
	...
	let last_sdk_entry = match fs::read_dir(&sdks_path) {
		...
		Err(_) => panic!(
			"Couldn't find SDK at {}, probably xcode is not installed",
			sdks_path.to_str().unwrap()
		),
	};

	let sdk = last_sdk_entry
		.path()
		.as_path()
		.file_stem()
		.unwrap()
...
fn build_windows(target: &str) {
	...
	.unwrap();
	let freeimage_proj = "FreeImage.2019.sln";
	...
	let out_dir = env::var("OUT_DIR").unwrap();
	let src_dir = freeimage_copy.join(platform).join(config);
